...
(define (derive-sum expr var)
  (define a1 (derive (first-operand expr) var))
  (define a2 (derive (second-operand expr) var))
  (make-sum a1 a2)
)
...
(define (derive-product expr var)
  (define a1 (derive (first-operand expr) var))
  (define a2 (derive (second-operand expr) var))
  (make-sum (make-product a1 (second-operand expr)) (make-product (first-operand expr) a2))
)

; Exponentiations are represented as lists that start with ^.
(define (make-exp base exponent)
  (cond ((or (=number? base 1) (=number? exponent 1)) base)
        ((=number? exponent 0) 1)
