...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(first, MacroProcedure):   # PROBLEM 20 
            expr = first.apply_macro(rest, env) # macro
            return scheme_eval(expr, env)
        
        fn = lambda x: scheme_eval(x, env)
        args = rest.map(fn)
...
def eval_all(expressions, env):
    ...
    if expressions == nil:
        return None
    final = scheme_eval(expressions.first, env, expressions.rest is nil)  # tail context when expressions.rest is nil
    return final if expressions.rest is nil else eval_all(expressions.rest, env)
    # END PROBLEM 7
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        lambda_form = do_lambda_form(Pair(formals, expressions.rest), env)
        env.define(target, lambda_form)
        return target
        # END PROBLEM 9
    else:
...
def do_if_form(expressions, env):
    ...
    if is_true_primitive(scheme_eval(expressions.first, env)):
        current_expr = expressions.rest           # current_expr.rest can be replaced with True, since there's just one expression in #t or #f cases.                          
        return scheme_eval(current_expr.first, env, current_expr.rest is nil) # tail context when current_expr.rest is nil
    elif len(expressions) == 3:
        current_expr = expressions.rest.rest
        return scheme_eval(current_expr.first, env, current_expr.rest is nil) # tail context when current_expr.rest is nil
...
def do_and_form(expressions, env):
    ...
    while expressions != nil:
        result = scheme_eval(expressions.first, env, expressions.rest is nil) # tail context when expressions.rest is nil
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    "*** YOUR CODE HERE ***"
    while bindings != nil:              # bindings   -->((x 2) (y 3))
        name_value = bindings.first     # name_value --> (x 2) 
        validate_form(name_value, 2, 2)                                         # name_value.rest       --> (2) "Error: int is not callable"
        name, value = name_value.first, scheme_eval(name_value.rest.first, env) # name_value.rest.first -->  2 
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        target, formals = target.first, target.rest
        lambda_form = MacroProcedure(formals, expressions.rest, env)
        env.define(target, lambda_form)
        return target
    else:
        bad_target = target.first if isinstance(target, Pair) else target
...
def complete_apply(procedure, args, env):
    ...
    validate_procedure(procedure)
    val = scheme_apply(procedure, args, env) # the args hasn't been eval for now, it's applyed straightforward.
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        """
        if tail and not scheme_symbolp(expr) and not self_evaluating(expr): # tail call, which is the last call for now cause its rest is nil, 
            return Thunk(expr, env)                                         # its info should be saved in thunk and returned as a temporary answer.
        result = Thunk(expr, env)                                           # so the whole evaluation can get rid of privious frame and restart from the temporary answer.
        # BEGIN PROBLEM 19
        while isinstance(result, Thunk):
            result = original_scheme_eval(result.expr, result.env) # tail call be updated here as a temporary answer, and then be evaluated since it's still a Thunk.
        return result                                               
        # END PROBLEM 19
