...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            ant, place_ant = self, place.ant 
            for _ in range(2):
                if isinstance(place_ant, ContainerAnt) and place_ant.can_contain(ant):
                    place_ant.contain_ant(ant)
                    place.ant = place_ant
                    [Insect.add_to(ant, place) for ant in [ant, place_ant]]
                    return
                ant, place_ant = place_ant, ant
            assert place.ant is None, 'Two ants in {0}'.format(place)
            # END Problem Optional 2
        Insect.add_to(self, place)

    def remove_from(self, place):         # 1. place.ant = None and 2. self.place = None (double removed)
        ...
        Insect.remove_from(self, place)   # 2.
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        def helper(new_place, distance):
            if new_place == beehive:
                return None
            elif new_place.bees and self.min_range <= distance <= self.max_range:
                return rANTdom_else_none(new_place.bees)
            else:
                return helper(new_place.entrance, distance + 1)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if not current_place.ant:
            amount += self.damage                   # increment the damage if the fire ant dies.
        damage_each_bee_in_the_place(current_place, amount)
        
def damage_each_bee_in_the_place(place, damage):
    for bee in place.bees[:]:                       # iterate over the list copy cause the dead bee will mutate the list
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        ...
        "*** YOUR CODE HERE ***"
        if bee:
            poor_bee = rANTdom_else_none(bee)
            Insect.reduce_armor(poor_bee, poor_bee.armor)
...
# END Problem EC
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        Ant.reduce_armor(self, amount)
        if self.is_true and not self.place:
            bees_win()
    
    def remove_from(self, place):
        if not self.is_true or self.armor <= 0:
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem Optional 1
        "*** YOUR CODE HERE ***"
        damage_each_bee_in_the_place(self.place, self.damage)
        # END Problem Optional 1
...
def make_slow(action, bee):
    ...
    "*** YOUR CODE HERE ***"
    def make_slow_action(gamestate):
...
def make_scare(action, bee):
    ...
    # BEGIN Problem Optional 4
    "*** YOUR CODE HERE ***"
    def make_scare_action(gamestate):
        bee.scared = True
        action(gamestate)
        bee.scared = False
    return make_scare_action
    # END Problem Optional 4
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem Optional 4
        "*** YOUR CODE HERE ***"
        if target and target not in self.scared_one:
            apply_status(make_scare, target, 2)
            self.scared_one.append(target)
        # END Problem Optional 4
    ...
    def insects_in_front(self, beehive):
        ...
        while place is not beehive:
            if place.ant and place.ant is not self:
                ant = place.ant
                insects_dict.update({ant:distance})
            for bee in place.bees:
                insects_dict.update({bee:distance})
