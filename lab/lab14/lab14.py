...
def num_splits(s, d):
    ...
    12
    """
    "*** YOUR CODE HERE ***"                   # -2   <= -2+3 -1 <= 2
    def helper(s, min, max):                   # -2+2 <=    3 -1 <= 2+2  # with_first    == -2+3 -1
        if len(s) == 1:                        # -2-2 <=    3 -1 <= 2-2  # without_first ==    3   -1-(-2) 
            return 1 if min <= s[0] <= max else 0
        return helper(s[1:], min-s[0], max-s[0]) + helper(s[1:], min+s[0], max+s[0])
    return helper(s, -d, d)
...
class CheckingAccount(Account):
    ...
    def deposit_check(self, check):
        if check.name == self.holder and not check.deposited:
            self.deposit(check.num)
            check.deposited = True
            return check.num
        else: print('The police have been notified.')
...
def filterl(lst, pred):
    ...
    def fn(z, num):
        if z is Link.empty:
            return Link(num) if pred(num) else z
        else:
            return Link(z.first, fn(z.rest, num))
